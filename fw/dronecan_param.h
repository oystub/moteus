...
/// A templated class for DroneCAN parameters supporting integers, floats, and
/// bools.
...
public:
  ...
  using PostUpdateCallback =
      std::function<void(const Parameter<T> &, const T &)>;
  ...
  Parameter(const std::string &name, T &value, T default_value = T{},
      ...
      : name_(name), value_(value), default_value_(default_value),
        min_value_(min_value), max_value_(max_value), pre_update_(pre_update),
        post_update_(post_update) {}

  /// Attempts to set the parameter's value, enforcing constraints and calling
  /// callbacks.
  ...
  uavcan_protocol_param_GetSetResponse Get() const {
    ...
    std::copy(name_.begin(), name_.begin() + response.name.len,
              response.name.data);
...
private:
  ...
  static void setValue(uavcan_protocol_param_Value &field,
  ...
  static void setValue(uavcan_protocol_param_NumericValue &field,
                       const std::optional<T> &val) {
...
#define DRONECAN_PARAMETER(name, variable, ...)                                \
  store.Register(Parameter<std::decay_t<decltype(variable)>>(#name, variable,  \
                                                             ##__VA_ARGS__))
