...
class DroneCanRotor {
public:
    DroneCanRotor(moteus::MoteusController* controller);

    void handle_esc_RawCommand(const uavcan_equipment_esc_RawCommand& msg);
    void handle_actuator_ArrayCommand(const uavcan_equipment_actuator_ArrayCommand& msg);
...
private:
    ...
    struct Config {
        ...
        int cartesian_cmd_idx[AxisIdx::AXIS_COUNT] = {0, 1, -1, -1, -1, 2};
        ...
        float min_actuation_thrust = 0.7f; // Minimum actuation thrust where full x and y actuation should be possible.
    
        template <typename Store>
        void RegisterParameters(Store& store) {
            DRONECAN_PARAMETER(ROT_CMD_MODE, command_mode, 1, 0, 2);
            DRONECAN_PARAMETER(ROT_DIR, direction, 1, -1, 1);
            ...
            DRONECAN_PARAMETER(ROT_ELV_IDX, polar_idx[PolarIdx::ELEVATION], 2, -1, 255);
            ...
            DRONECAN_PARAMETER(ROT_THR_MINACT, min_actuation_thrust, 0.7f, 0.0f, 1.0f);
        }
    ...
    static inline float normalizeEsc(int16_t raw) {
        ...
        return (raw >= 0) ? (raw / 8191.0f) : (raw / 8192.0f);
    }
    static inline bool validIdx(int idx, int len) { return idx >= 0 && idx < len; }
    static inline float deg2rad(float d) { return d * std::numbers::pi_v<float> / 180.0f; }
    ...
    void handleCartesianActuatorArray(const uavcan_equipment_actuator_ArrayCommand& msg);
    void handlePolarActuatorArray(const uavcan_equipment_actuator_ArrayCommand& msg);
