...
public:
  ...
  template <typename T> void Register(Parameter<T> param) {
    auto ptr =
        mjlib::micro::PoolPtr<ParamWrapper<T>>(pool_, param, next_index_++);
    params_.push_back(
        ptr.get()); // Store the base pointer for polymorphic access
  }

  /// Registers all parameters within a struct that calls DRONECAN_PARAMETER
  /// inside `RegisterParameters()`
  ...
  template <typename T> bool SetValueByIndex(size_t index, T new_value) {
  ...
  GetSet(const uavcan_protocol_param_GetSetRequest &req) {
    bool is_set_request =
    ...
    std::string dronecan_name(reinterpret_cast<const char *>(req.name.data),
                              name_len);
  ...
  void Sort() {
    std::sort(params_.begin(), params_.end(), [](const auto &a, const auto &b) {
...
private:
  template <typename T> struct ParamWrapper : public BaseParamWrapper {
    explicit ParamWrapper(Parameter<T> param, size_t idx) : param_(param) {
      index = idx;
    }

    const std::string &GetName() const override { return param_.GetName(); }

    uavcan_protocol_param_GetSetResponse Get() override { return param_.Get(); }

    bool Set(const uavcan_protocol_param_Value &req) override {
      return param_.Set(req);
    }

    void Reset() override { param_.Reset(); }
